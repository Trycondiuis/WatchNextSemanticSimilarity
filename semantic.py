...
tokens = nlp('car skateboard train bicycle ')
...
# note:
# It is quite interesting when there is an identical match between the words, we get an integer (1) this means
# that the more distance the two objects being compared have the more different they will be, which means that
# Similarity it’s subjective and highly dependent on how it's been used. So, special care should be taken
# when calculating/comparing similarity across objects or other items that are unrelated to each other or not
# relevant to the problem.

# if we change the tokens to 'car boat train plane ' we will have different values with very close similarity between
# them as they are considered a way of transportation, but if we change two of the tokens for 'skateboard bicycles'
# we get that 'car - skateboard', 'skateboard - train' aren't similar despite being a way of transportation,
# but skateboard it's not commonly use as long distance transportation or transportation for masses.
# Somehow the system assume they are a way of transportation, but with little similarities
...
# note:
# when using simpler language model, the similarities dropped drastically, and we get a UserWarning: [W007]
# The model you're using has no word vectors loaded. This is because the simpler method is based on the tagger, parser,
# and NER. These do not give useful judgement on similarities, as the model do not use word vectors. It is
# context-sensitive only, drastically reducing the similarities between longer texts comparison. For the latest
# it is better to use the larger models instead like the 'en_core_web_md'.
